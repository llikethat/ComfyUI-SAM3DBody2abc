...
console.log("[SAM3DBody2abc_FBXAnimationViewer] Loading FBX Animation Viewer extension");
    ...
    <script type="module">
        ...
        function loadFBX(path) {
            ...
            loader.load(
                url,
                (fbx) => {
                    console.log('[SAM3DBody2abc_FBXAnimationViewer] FBX loaded successfully');
                    console.log('[SAM3DBody2abc_FBXAnimationViewer] === FBX STRUCTURE ===');
                    console.log('[SAM3DBody2abc_FBXAnimationViewer] Root name: ' + fbx.name);
                    console.log('[SAM3DBody2abc_FBXAnimationViewer] Root type: ' + fbx.type);
                    console.log('[SAM3DBody2abc_FBXAnimationViewer] Direct children: ' + fbx.children.length);

                    fbx.children.forEach((child, i) => {
                        console.log('[SAM3DBody2abc_FBXAnimationViewer] Child[' + i + ']: ' + child.name + ' (type: ' + child.type + ')');
                    });
                    ...
                    fbx.traverse((child) => {
                        if (child.isMesh) {
                            ...
                            console.log('[SAM3DBody2abc_FBXAnimationViewer] === MESH DEBUG ===');
                            console.log('[SAM3DBody2abc_FBXAnimationViewer] Name: ' + child.name);
                            console.log('[SAM3DBody2abc_FBXAnimationViewer] Vertices: ' + vertCount);
                            console.log('[SAM3DBody2abc_FBXAnimationViewer] Is SkinnedMesh: ' + isSkinned);
                            console.log('[SAM3DBody2abc_FBXAnimationViewer] Visible: ' + child.visible);

                            // Material info
                            const materials = Array.isArray(child.material) ? child.material : [child.material];
                            console.log('[SAM3DBody2abc_FBXAnimationViewer] Material count: ' + materials.length);

                            materials.forEach((mat, i) => {
                                if (mat) {
                                    console.log('[SAM3DBody2abc_FBXAnimationViewer] Material[' + i + ']: ' + mat.name + ' (type: ' + mat.type + ')');
                                    console.log('[SAM3DBody2abc_FBXAnimationViewer]   Color: ' + (mat.color ? mat.color.getHexString() : 'none'));
                                    console.log('[SAM3DBody2abc_FBXAnimationViewer]   Map (diffuse): ' + (mat.map ? mat.map.name || 'yes' : 'none'));
                                    console.log('[SAM3DBody2abc_FBXAnimationViewer]   NormalMap: ' + (mat.normalMap ? 'yes' : 'none'));
                                    console.log('[SAM3DBody2abc_FBXAnimationViewer]   Transparent: ' + mat.transparent);
                                    console.log('[SAM3DBody2abc_FBXAnimationViewer]   Opacity: ' + mat.opacity);
                                    console.log('[SAM3DBody2abc_FBXAnimationViewer]   Side: ' + mat.side);
                                } else {
                                    console.log('[SAM3DBody2abc_FBXAnimationViewer] Material[' + i + ']: NULL');
                                }
                            });
                        }
                    });
                    console.log('[SAM3DBody2abc_FBXAnimationViewer] Total meshes: ' + meshCount + ', SkinnedMeshes: ' + skinnedMeshCount);
                    ...
                    if (skinnedMesh && skinnedMesh.skeleton) {
                        console.log('[SAM3DBody2abc_FBXAnimationViewer] Found skeleton on: ' + skinnedMesh.name);
                        console.log('[SAM3DBody2abc_FBXAnimationViewer] Skeleton bones: ' + skinnedMesh.skeleton.bones.length);
                        skeletonHelper = new THREE.SkeletonHelper(fbx);
                        skeletonHelper.material.linewidth = 2;
                        skeletonHelper.visible = showSkeleton;
                        scene.add(skeletonHelper);
                    } else {
                        console.warn('[SAM3DBody2abc_FBXAnimationViewer] No skinned mesh with skeleton found');
                    }

                    // Setup animations
                    animations = fbx.animations || [];
                    console.log('[SAM3DBody2abc_FBXAnimationViewer] Found ' + animations.length + ' animation(s)');

                    if (animations.length > 0) {
                        setupAnimations();
                    } else {
                        console.warn('[SAM3DBody2abc_FBXAnimationViewer] No animations found in FBX');
                ...
                (error) => {
                    console.error('[SAM3DBody2abc_FBXAnimationViewer] Error loading FBX:', error);
...
app.registerExtension({
    name: "Comfy.MotionCapture.SAM3DBody2abc_FBXAnimationViewer",

    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "SAM3DBody2abc_FBXAnimationViewer") {
            console.log("[SAM3DBody2abc_FBXAnimationViewer] Registering SAM3DBody2abc_FBXAnimationViewer node");
            ...
            nodeType.prototype.onNodeCreated = function() {
                ...
                console.log("[SAM3DBody2abc_FBXAnimationViewer] Node created, adding unified widget");
                ...
                window.addEventListener('message', (event) => {
                    ...
                    switch(type) {
                        case 'VIEWER_READY':
                            console.log("[SAM3DBody2abc_FBXAnimationViewer] Viewer ready");
                        ...
                        case 'ANIMATIONS_LOADED':
                            console.log("[SAM3DBody2abc_FBXAnimationViewer] ✓ Animations loaded successfully!");
                        ...
                        case 'NO_ANIMATIONS':
                            console.warn("[SAM3DBody2abc_FBXAnimationViewer] ⚠ No animations found in FBX file");
                ...
                console.log("[SAM3DBody2abc_FBXAnimationViewer] Node setup complete");
                return result;
            };

            // Add method to load FBX
            nodeType.prototype.loadAnimationInViewer = function(fbxPath) {
                console.log("[SAM3DBody2abc_FBXAnimationViewer] loadAnimationInViewer called with:", fbxPath);

                if (!this.animationViewerIframe || !this.animationViewerIframe.contentWindow) {
                    console.warn("[SAM3DBody2abc_FBXAnimationViewer] Iframe not ready, deferring load");
                    this.fbxPathToLoad = fbxPath;
                    return;
                }

                if (!this.animationViewerReady) {
                    console.log("[SAM3DBody2abc_FBXAnimationViewer] Viewer not ready yet, deferring load");
                    this.fbxPathToLoad = fbxPath;
                    return;
                }
                ...
                console.log("[SAM3DBody2abc_FBXAnimationViewer] Sending LOAD_FBX message to iframe");
                console.log("[SAM3DBody2abc_FBXAnimationViewer] Relative path:", relativePath);
                console.log("[SAM3DBody2abc_FBXAnimationViewer] View URL:", viewPath);
            ...
            nodeType.prototype.onExecuted = function(message) {
                console.log("[SAM3DBody2abc_FBXAnimationViewer] onExecuted called");
                console.log("[SAM3DBody2abc_FBXAnimationViewer] Message:", message);
                ...
                if (message?.fbx_path?.[0]) {
                    ...
                    console.log("[SAM3DBody2abc_FBXAnimationViewer] ✓ Node executed with FBX path:", fbxPath);
                    this.loadAnimationInViewer(fbxPath);
                } else {
                    console.warn("[SAM3DBody2abc_FBXAnimationViewer] ⚠ No fbx_path in message");
                    console.warn("[SAM3DBody2abc_FBXAnimationViewer] Available keys:", Object.keys(message || {}));
                    console.warn("[SAM3DBody2abc_FBXAnimationViewer] Full message:", message);
                }
            };
        }
    }
...
console.log("[SAM3DBody2abc_FBXAnimationViewer] Extension registered successfully");